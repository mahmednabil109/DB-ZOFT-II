...
public class IndexUpdateAndDeleteTests {
    ...
    @Test
    @Order(6)
    public void testDeletionFromIndex() throws Exception{
        final DBApp dbApp = new DBApp();
        dbApp.init();
        ...
        htb.put("gpa", new Double(1.31));

        Assertions.assertDoesNotThrow( () ->
            dbApp.deleteFromTable("students", htb)
        );

        itr = dbApp.selectFromTable(
            new SQLTerm [] {
                t2
            }, 
            new String[0]
        ...
        Vector<Tuple> res2 = new Vector<>();

        while(itr.hasNext()) res2.add((Tuple) itr.next());
    ...
    @Test
    @Order(7)
    public void testUpdateInIndex() throws Exception{
        final DBApp dbApp = new DBApp();
        dbApp.init();
        ...
        Vector<Tuple> res1 = new Vector<>();
        Vector<Tuple> res2 = new Vector<>();

        while(itr1.hasNext()) res1.add((Tuple) itr1.next());

        int prevSize = res1.size();

        Hashtable<String, Object> htb = new Hashtable<>();
        htb.put("gpa", new Double(2));

        Assertions.assertDoesNotThrow( () ->{
            dbApp.updateTable("students", "86-4022", htb);
            dbApp.updateTable("students", "74-3211", htb);
            // dbApp.updateTable("students", "63-5881", htb);
        });


        itr1 = dbApp.selectFromTable(
            new SQLTerm [] {
                t1
            },
            new String[0]
        ...
        while(itr1.hasNext()) res2.add((Tuple) itr1.next());

        Assertions.assertEquals(prevSize + 2, res2.size());
